...
class LinksStock {
    ...
    public LinksStock(int max, CustomQueue<String> toVisit,boolean update) {
        this.maxCount = max;
        this.toVisit.addAll(toVisit);
        time = new Timestamp(System.currentTimeMillis());
        toVisit.getQueue().forEach((s)-> DBConnect.toVisitDB(s.toString()));
    }
    public LinksStock(int max, CustomQueue<String> toVisit,Set<String> auxilary,Set<String> visited,boolean update) {
        this.maxCount = max;
        this.toVisit.addAll(toVisit);
    ...
    public void visit(String link, Document Dom,long hoursCount) {
            visited.add(link);
            auxilary.remove(link);
            Timestamp newTime = new Timestamp(this.time.getTime()+(hoursCount*3600000));//
            DBConnect.visitDB(link,Dom.toString(),newTime,this.update);
...
//    }
 ...
 public void page() throws IOException {
      for(String s : visited)
      {
          ...
          Elements linksOnPage = pages.get(s)== null?  Jsoup.connect( s ).ignoreHttpErrors( true ).get().select("a[href]"): pages.get(s).select("a[href]");
